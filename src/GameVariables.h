...
public:
    ...
    template <typename T>
    inline T* getGameObject(GameObjectID id) const {
        auto ref = getObjectReference(id);
        return ref.index != INVALID_INDEX ? static_cast<T*>(gameObjects[ref.index]) : nullptr;
    }

    template <typename T>
    inline T* getGameObject(GameObjectRef& ref) const {
        updateObjectReference(ref);
        return ref.index != INVALID_INDEX ? static_cast<T*>(gameObjects[ref.index]) : nullptr;
    }
